...
import {getAlphaNumericPatternValidator} from "@app-shared/format.functions";
  ...
  providers: [
    {
      ...
      useExisting: forwardRef(() => InputAlphanumericDirective),
      multi: true
    }
  ]
})
export class InputAlphanumericDirective extends DefaultValueAccessor {
  @Input() inputEl;
  ...
  input(value) {
    ...
    if (element) {
      ...
      while (element.value.search(getAlphaNumericPatternValidator()) !== -1) {
        element.value = element.value.replace(getAlphaNumericPatternValidator(), '');
      }
